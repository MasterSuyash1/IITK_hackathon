...
const ChatBotModal = () => {
    ...
    // Set initial message when the modal opens
    useEffect(() => {
        if (isOpen) {
        ...
        if (inputValue.trim() !== "") {
            ...
            axios
                ...
                .then((response) => {
                    ...
                    setMessages((prev) => [
                        ...prev,
                        { text: response.data.response, isUser: false }, // Correct response key
                    ]);
                    setIsLoading(false);
                })
                .catch((error) => {
            ...
            // Clear input and reset speech transcript
            setInputValue("");
            resetTranscript();
        }
    };
    ...
    const handleVoiceInput = () => {
        if (SpeechRecognition.browserSupportsSpeechRecognition()) {
            if (!listening) {
                ...
                handleSendMessage(); // Send message when stopped listening
            }
        } else {
